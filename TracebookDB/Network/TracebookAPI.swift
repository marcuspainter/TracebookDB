...
class TracebookAPI {
    ...
    func getListResponse<T: Decodable>(_ type: T.Type, for request: URLRequest) async -> [T] {
        do {
            let (jsonData, _  ) = try await URLSession.shared.data(for: request)
            let response: [T] = try JSONDecoder().decode([T].self, from: jsonData)
            return response
        } catch {
    ...
    func getMeasurementListbyDate(cursor: Int = 0, fromDate: String) async -> MeasurementListResponse? {
        print(fromDate)
