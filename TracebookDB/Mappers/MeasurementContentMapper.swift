...
class MeasurementContentMapper {
    ...
    static func toModel(body: MeasurementContentBody) -> MeasurementContent? {
        ...
        let model = MeasurementContent(
            ...
            tfFrequency: DataMapper.parseDoubleArray(body.tfJSONFrequency ?? ""),
            tfMagnitude: DataMapper.parseDoubleArray(body.tfJSONMagnitude ?? ""),
            tfPhase: DataMapper.parseDoubleArray(body.tfJSONPhase ?? ""),
            tfCoherence: DataMapper.parseDoubleArray(body.tfJSONCoherence ?? ""),
